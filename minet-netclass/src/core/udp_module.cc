...
int main(int argc, char *argv[])
{
	...
	while (MinetGetNextEvent(event)==0) {
		cerr << " entered the top udp module loop" << endl;
		if (event.eventtype!=MinetEvent::Dataflow
			...
			if (event.handle==mux) {
				Packet p;
				cerr << " entered the mux udp module loop" << endl;
				unsigned short len;
				bool checksumok;
				MinetReceive(mux,p);
				...
				cerr << "the received packet is " << p << endl;
			}
			if (event.handle==sock) {
				cerr << " entered the sock udp module loop" << endl;
				...
				{ // ignored, send OK response
					cerr << "entered the accept case" << endl;
					...
					repl.error=EOK;
					MinetSend(sock,repl);
					cerr << "this connection was " << repl.connection << endl;
				}
				break;
				case STATUS:
				...
				case WRITE:
				{
					cerr << "entered the write case" << endl;
					...
					UDPHeader udph=p.FindHeader(Headers::UDPHeader);
					IPHeader iph=p.FindHeader(Headers::IPHeader);
					//TCPHeader tcph=p.FindHeader(Headers::TCPHeader);
					cerr << " the headers were: " << iph << " " << udph << endl;
					SockRequestResponse repl;
				...
				case FORWARD:
				{
					cerr << "entered the forward case" << endl;
